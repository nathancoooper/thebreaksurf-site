...
export default function ImageUpload({ label, value, onChange, meta }: Props) {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (meta) {
      ...
      if (meta.color) fd.append('color', meta.color);
      if (meta.view) fd.append('view', meta.view);
    }
  ...
  return (
    <div>
      ...
      {value && (
        <img
          src={cacheBust ? `${value}?t=${cacheBust}` : value}
          alt="Preview"
          className="mt-2 h-16 w-16 rounded-md border border-gray-200 object-cover bg-gray-100"
